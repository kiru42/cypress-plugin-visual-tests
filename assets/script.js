...
(function() {
  ...
  function readFile(fullPath, encoding = 'base64', options = { log: false, timeout: 2000 }) {
    ...
    if (!cache[fullPath]) {
      cache[fullPath] = Cypress.Promise.resolve(
        cy.readFile(fullPath, encoding, options).then(result => {
  ...
  function getImageDataUri(url) {
    return readFile(url, 'base64').then(image => {
  ...
  function wrapImage(type, title, dataUri = '', imageStyle = '') {
    ...
    return (
      `<div class="snapshot-image snapshot-image--${type}">${htmlTitle}` +
      wrapImageStyle(
        imageStyle,
  ...
  function formatImageResult(data) {
    ...
    if (data.passed || data.updated) {
      return getImageDataUri(data.expected.path).then(dataUri =>
        formatPreview(title, wrapImage('single', title, dataUri))
      );
    }

    const imageHeight = data.diff
      ? data.diff.height
      : Math.max(data.actual.height, data.expected.height);
    const imageWidth = data.diff
      ? data.diff.width
      : Math.max(data.actual.width, data.expected.width);
    const imageRatio = Math.round((imageHeight / imageWidth) * 10000) / 100;
    const imageStyle = `padding-top: ${imageRatio}%;`;

    const promises = [getImageDataUri(data.expected.path), getImageDataUri(data.actual.path)];
    if (data.diff) {
      promises.push(getImageDataUri(data.diff.path));
    }

    return Cypress.Promise.all(promises).then(([expectedImage, actualImage, diffImage = '']) => {
  ...
  function formatResult(data) {
    ...
    if (data.isImage) {
      ...
      return Cypress.Promise.resolve(
        Diff2Html.getPrettyHtml(data.diff, {
          inputFormat: 'diff',
  ...
  Cypress.$(document.body).on('click', 'a[href^="#cypress-plugin-snapshot-"]', function(e) {
    ...
    const data = JSON.parse(
      Base64.decode(e.currentTarget.getAttribute('href').replace('#cypress-plugin-snapshot-', ''))
    );
    console.log('data:', data);
    if (data) {
      formatResult(data).then(diffHtml => {
        const updateButton =
          window.saveSnapshot && !data.passed && !data.updated
            ? '<button class="snapshot-btn-approve"><i class="fa fa-check"></i> Update snapshot</button>'
            : '';

        Cypress.$(
          `<div class="snapshot-diff${data.isImage ? ' snapshot-diff--image' : ''}">` +
            '<header>' +
            updateButton +
            '<button class="snapshot-btn-close"><i class="fa fa-close"></i> Close</button>' +
            '</header>' +
            '<div class="snapshot-diff-content">' +
            diffHtml +
            '</div>' +
            '</div>'
